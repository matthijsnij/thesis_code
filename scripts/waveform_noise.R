...
for (r in 1:reps) {
  ...
  wave_X_train_noise <- matrix(rnorm(nrow(wave_X_train_real) * num_noise), nrow = nrow(wave_X_train_real), ncol = num_noise)
  wave_X_test_noise <- matrix(rnorm(nrow(wave_X_test_real) * num_noise), nrow = nrow(wave_X_test_real), ncol = num_noise)
  ...
  # run rf
  p <- ncol(wave_X_train)
  mtry_grid <- c(2, 4, 6, 8, 10, 20, 30, 40)
